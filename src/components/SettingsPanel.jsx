...
export default function SettingsPanel({ 
  ...
  // Tab & Frequency States
  const [activeTab, setActiveTab] = useState('global'); // 'global' | 'trip-planner' | 'stock-lookout' | 'news-briefing'
  const [tripFrequency, setTripFrequency] = useState('daily');
  const [stockFrequency, setStockFrequency] = useState('daily');
  const [newsFrequency, setNewsFrequency] = useState('daily');
  const [updatingFrequency, setUpdatingFrequency] = useState(false);

  useEffect(() => {
    if (quizContext) {
      setTripFrequency(quizContext.feed_frequency || 'daily');
    }
  }, [quizContext]);

  useEffect(() => {
    if (stockQuizContext) {
      setStockFrequency(stockQuizContext.feed_frequency || 'daily');
    }
  }, [stockQuizContext]);

  useEffect(() => {
    if (newsQuizContext) {
      setNewsFrequency(newsQuizContext.feed_frequency || 'daily');
    }
  }, [newsQuizContext]);

  const handleFrequencyChange = async (fishSlug, val) => {
    if (fishSlug === 'trip-planner') setTripFrequency(val);
    else if (fishSlug === 'stock-lookout') setStockFrequency(val);
    else if (fishSlug === 'news-briefing') setNewsFrequency(val);

    setUpdatingFrequency(true);
    try {
      if (onUpdateFeedFrequency) {
        await onUpdateFeedFrequency(fishSlug, val);
      }
      setStatusMessage({ type: 'success', text: 'Feed update frequency saved!' });
    } catch (err) {
      console.error(err);
      setStatusMessage({ type: 'error', text: 'Failed to update feed frequency.' });
    } finally {
  ...
  const renderPreferencesTable = (context) => {
    ...
    const filtered = Object.entries(context).filter(([key]) => key !== 'feed_frequency');
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Tabs Header */}
            <div className="px-6 py-2 bg-black/25 border-b border-white/5 flex gap-1 shrink-0 overflow-x-auto select-none no-scrollbar">
              <button
                type="button"
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-all whitespace-nowrap ${
                  activeTab === 'global'
                    ...
                    : 'text-sea-foam/50 hover:text-white border border-transparent'
                }`}
              >
                ⚙️ Global
              </button>
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-all whitespace-nowrap ${
                  activeTab === 'trip-planner'
                    ? 'bg-coral-warm/15 text-coral-warm border border-coral-warm/30'
                    : 'text-sea-foam/50 hover:text-white border border-transparent'
                }`}
              >
                🐠 Trip Planner
              ...
              <button
                type="button"
                onClick={() => setActiveTab('stock-lookout')}
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-all whitespace-nowrap ${
                  activeTab === 'stock-lookout'
              ...
              <button
                type="button"
                onClick={() => setActiveTab('news-briefing')}
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg flex items-center gap-1.5 transition-all whitespace-nowrap ${
                  activeTab === 'news-briefing'
                    ? 'bg-[#00e673]/15 text-[#00e673] border border-[#00e673]/30'
                    : 'text-sea-foam/50 hover:text-white border border-transparent'
                }`}
              >
                📰 World News
            ...
            {/* Content list */}
            <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar pb-12">
              <AnimatePresence mode="wait">
                {activeTab === 'global' && (
                  ...
                  >
                    ...
                    {statusMessage && (
                      <div className={`p-3.5 rounded-lg border text-xs font-mono flex items-start gap-2.5 bg-white/[0.01] ${
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <form onSubmit={handleSaveApiKeys} className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-4">
                        ...
                        <button
                          type="submit"
                          disabled={savingKeys}
                          className="w-full py-2 bg-bioluminescent text-black font-semibold rounded text-xs hover:shadow-[0_0_10px_rgba(0,229,255,0.3)] active:scale-98 transition-all disabled:opacity-50 font-sans"
                        >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-semibold text-white font-sans">Enable PIN Protection</p>
                            <p className="text-xs text-sea-foam/55 font-sans">Require 4-digit PIN lock screen on launch.</p>
                          </div>
                          <button
                            type="button"
                            onClick={onTogglePin}
                            className={`premium-switch-track ${pinEnabled ? 'bg-bioluminescent/25 border-bioluminescent/40' : 'bg-white/5 border-white/10'}`}
                          >
                            <div 
                              className={`premium-switch-handle ${pinEnabled ? 'translate-x-5 bg-bioluminescent shadow-[0_0_8px_rgba(0,229,255,0.6)]' : 'translate-x-0 bg-white/40'}`} 
                            />
                          </button>
                        </div>
                          ...
                          <div className="pt-2 border-t border-white/5 space-y-3">
                            ...
                            {!showPinInput ? (
                              <button
                                ...
                                className="text-xs text-bioluminescent hover:underline flex items-center gap-1 font-sans"
                              >
                                <Key size={12} />
                                Change PIN Code
                              </button>
                              ...
                              <form onSubmit={handlePinUpdateSubmit} className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                  <button
                                    type="submit"
                                    className="px-3 py-1 rounded bg-white/10 text-[10px] text-white hover:bg-white/20 font-sans"
                                  >
                                    Save
                                  </button>
                                  <button
                                    ...
                                    className="px-3 py-1 rounded text-[10px] text-sea-foam/45 hover:text-white font-sans"
                                  >
                                    Cancel
                                  </button>
                                </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 text-xs text-sea-foam/60 space-y-2 leading-relaxed font-sans">
                        <p>
                          <span className="text-white font-semibold">Supabase Postgres</span> - Holds database tables and tracks agent logs.
                        </p>
                        <p>
                          <span className="text-white font-semibold">Gemini 2.5 Flash</span> - Generates curated suggestions matching profile constraints.
                        </p>
                        <p>
                    ...
                    <div className="pt-4 border-t border-white/5">
                      <button
                        type="button"
                        onClick={onLogout}
                        className="w-full py-3 rounded-lg border border-red-500/20 bg-red-950/10 hover:bg-red-950/20 text-red-400 font-semibold text-xs flex items-center justify-center gap-1.5 transition-all duration-200 font-sans"
                      >
                        <LogOut size={14} />
                        Sign Out of Account
                      </button>
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <h4 className="font-mono text-[10px] uppercase tracking-wider text-coral-warm font-bold flex items-center gap-1.5">
                        <span>🐠</span> Trip Planner Curation
                      </h4>
                      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-4">
                        <p className="text-xs text-sea-foam/70 leading-relaxed font-sans">
                          Force curating travel options. Uses Tavily to scrape flight patterns and forums for trip destinations, generating content via Gemini 2.5 Flash.
                        </p>
                        ...
                        <button
                          ...
                          className="w-full py-2.5 rounded-lg bg-bioluminescent text-black font-semibold text-sm flex items-center justify-center gap-1.5 hover:shadow-[0_0_15px_rgba(0,229,255,0.4)] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-sans"
                        >
                          {loadingTrip ? (
                            <>
                    ...
                    {/* Feed Frequency */}
                    <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-3">
                      <label className="text-[10px] uppercase font-mono tracking-wider text-sea-foam/50 block">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => handleFrequencyChange('trip-planner', e.target.value)}
                          className="w-full bg-white/[0.03] border border-white/10 rounded px-3 py-2 text-xs font-sans text-white focus:outline-none focus:border-bioluminescent appearance-none cursor-pointer pr-8"
                        >
                          <option value="daily" className="bg-[#050d1a] text-white">Daily (Default)</option>
                          <option value="twice_daily" className="bg-[#050d1a] text-white">Twice Daily</option>
                          <option value="every_2_days" className="bg-[#050d1a] text-white">Every 2 Days</option>
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-sea-foam/50">
                          <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                        </div>
                      </div>
                      <p className="text-[10px] text-sea-foam/40 leading-relaxed font-sans">
                        Determines how often the background agent swimmer triggers digest updates.
                      </p>
                    </div>
                ...
                {activeTab === 'stock-lookout' && (
                  ...
                  >
                    ...
                    {/* Stock Manual Trigger */}
                    <div className="space-y-4">
                      <h4 className="font-mono text-[10px] uppercase tracking-wider text-[#42a5f5] font-bold flex items-center gap-1.5">
                        <span>🐠</span> Stock Lookout Curation
                      </h4>
                      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-4">
                        <p className="text-xs text-sea-foam/70 leading-relaxed font-sans">
                          Force researching the stock market. Tavily executes 4 financial sentiment queries, compiling analysis cards, confidence scores, and themes via Gemini 2.5 Flash.
                        </p>
                        ...
                        <button
                          ...
                          disabled={loadingTrip || loadingStock || loadingNews || energy === 0}
                          className="w-full py-2.5 rounded-lg bg-[#1e88e5] text-white font-semibold text-sm flex items-center justify-center gap-1.5 hover:shadow-[0_0_15px_rgba(30,136,229,0.4)] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-sans"
                        >
                          {loadingStock ? (
                            <>
                              <Loader2 size={16} className="animate-spin" />
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-3">
                      ...
                      <div className="relative">
                        <select
                          value={stockFrequency}
                          disabled={updatingFrequency}
                          onChange={(e) => handleFrequencyChange('stock-lookout', e.target.value)}
                          className="w-full bg-white/[0.03] border border-white/10 rounded px-3 py-2 text-xs font-sans text-white focus:outline-none focus:border-bioluminescent appearance-none cursor-pointer pr-8"
                        >
                          <option value="daily" className="bg-[#050d1a] text-white">Daily (Default)</option>
                          <option value="twice_daily" className="bg-[#050d1a] text-white">Twice Daily</option>
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <h4 className="font-mono text-[10px] uppercase tracking-wider text-[#00e673] font-bold flex items-center gap-1.5">
                        <span>📰</span> World News Curation
                      </h4>
                      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-4">
                        <p className="text-xs text-sea-foam/70 leading-relaxed font-sans">
                          Force compiling world news briefing. Tavily runs searches against global news indexes for today's date, generating story cards and summaries via Gemini 2.5 Flash.
                        </p>
                        ...
                        <button
                          ...
                          disabled={loadingTrip || loadingStock || loadingNews || energy === 0}
                          className="w-full py-2.5 rounded-lg bg-[#00e673] text-black font-semibold text-sm flex items-center justify-center gap-1.5 hover:shadow-[0_0_15px_rgba(0,230,115,0.4)] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-sans"
                        >
                          {loadingNews ? (
                            <>
                              <Loader2 size={16} className="animate-spin" />
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 space-y-3">
                      ...
                      <div className="relative">
                        <select
                          value={newsFrequency}
                          disabled={updatingFrequency}
                          onChange={(e) => handleFrequencyChange('news-briefing', e.target.value)}
                          className="w-full bg-white/[0.03] border border-white/10 rounded px-3 py-2 text-xs font-sans text-white focus:outline-none focus:border-bioluminescent appearance-none cursor-pointer pr-8"
                        >
                          <option value="daily" className="bg-[#050d1a] text-white">Daily (Default)</option>
                          <option value="twice_daily" className="bg-[#050d1a] text-white">Twice Daily</option>
